(ns onyx-batch-fix.catalogs.sample-catalog)
  ...
  (mapv
   (fn [entry]
     (cond (and (some #{(:onyx/name entry)} tasks) (= (:onyx/type entry) :input))
           (merge
            entry
            {:onyx/plugin :core.async/read-from-chan
             :onyx/type :input
           ...
           (merge
            entry
            {:onyx/plugin :core.async/write-to-chan
             :onyx/type :output
             :onyx/medium :core.async
             :onyx/max-peers 1
  ...
  ([batch-size batch-timeout]
     [{:onyx/name :read-lines
      ...
      {:onyx/name :format-line
       :onyx/fn :onyx-batch-fix.functions.sample-functions/format-line
       :onyx/type :function
       :onyx/batch-size batch-size
       :onyx/batch-timeout batch-timeout
      ...
      {:onyx/name :upper-case
       :onyx/fn :onyx-batch-fix.functions.sample-functions/upper-case
       :onyx/type :function
       :onyx/batch-size batch-size
       :onyx/batch-timeout batch-timeout
