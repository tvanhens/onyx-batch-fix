(ns onyx-batch-fix.lifecycles.sample-lifecycle
    ...
    (fn [{:keys [lifecycle/task lifecycle/replaceable?] :as lifecycle}]
      (let [catalog-entry (u/find-task catalog task)]
        (cond (and (some #{task} tasks) replaceable?
              ...
              [{:lifecycle/task task
                :lifecycle/calls :onyx-batch-fix.lifecycles.sample-lifecycle/in-calls
              ...
              [{:lifecycle/task task
                :lifecycle/calls :onyx-batch-fix.lifecycles.sample-lifecycle/out-calls
                :core.async/id (java.util.UUID/randomUUID)}
               {:lifecycle/task task
...
(defn build-lifecycles []
  [{:lifecycle/task :read-lines
    :lifecycle/calls :onyx-batch-fix.plugins.http-reader/reader-calls
   ...
   {:lifecycle/task :write-lines
    :lifecycle/calls :onyx-batch-fix.lifecycles.sample-lifecycle/log-calls
